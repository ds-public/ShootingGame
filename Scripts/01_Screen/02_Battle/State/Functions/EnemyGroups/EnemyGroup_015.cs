...
namespace Sample_001
{
	public partial class Battle
	{
		...
		/// エネミーグループ(種別 015)
		/// </summary>
		public class EnemyGroup_015 : EnemyGroupBase
		{
			/// <summary>
			/// 画面中央にアルファフェードで黒い表示状態でランダムに移動する
			/// </summary>
...
//				variationType = 1 ;	// デバッグ
				...
				for( i  = 0 ; i <  l ; i ++ )
				{
					...
					owner.CreateEnemy( EnemyShapeTypes.No_015, 2, shield, 100, groupId, OnEnemyUpdate, OnEnemyDestroyed, settings, level ) ;
...
//				bool autoRotation = false ;
				...
				// 単色化
				enemy.SetMonochromaticMode( true, 1, 0x3F000000 ) ;
				...
				while( true  )	// 画面内の座標割合値で位置を判定する
				{
					...
					if( phase == 1 )
					{
						...
						if( factor >= 1 )
						{
							...
							// 現在位置によってランダムの進行方向を変える(画面外に出さないため)
							Vector2 cp = enemy.RatioPosition ;
							Vector2 np ;

							if( cp.X <  -0.4f || cp.Y <  -0.4f || cp.X >  +0.4f || cp.Y >  +0.4f )
							{
								// 画面の外側にいる(ランダム範囲を狭く)
								float nx = ExMath.GetRandomRange( -0.35f, +0.35f ) ;
								float ny = ExMath.GetRandomRange( -0.35f, +0.35f ) ;

								np = new Vector2( nx, ny ) ;
							}
							else
							{
								// 画面の内側にいる(ランダム範囲を広く)
								float nx = ExMath.GetRandomRange( -0.45f, +0.45f ) ;
								float ny = ExMath.GetRandomRange( -0.45f, +0.45f ) ;
							...
							if( velocity.LengthSquared() >  0 )
							{
								...
								basePosition = enemy.Position ;
								movePosition = basePosition + direction * 100.0f ;
					...
					if( phase == 2 )
					{
						...
						velocity = movePosition - basePosition ;

						enemy.Position = velocity * factor + basePosition ;
