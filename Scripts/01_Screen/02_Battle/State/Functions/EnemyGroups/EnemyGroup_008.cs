...
namespace Sample_001
{
	public partial class Battle
	{
		...
		/// エネミーグループ(種別 008)
		/// </summary>
		public class EnemyGroup_008 : EnemyGroupBase
		{
			/// <summary>
			/// 画面外周を∪字に移動
			/// </summary>
...
//				isDouble = true ;	// デバッグ
			...
			private async Task OnEnemyUpdate( Enemy enemy, CancellationToken linkedToken )
			{
				...
				if( variationType == 0 )
				{
					...
					checkRatioPosition = new Vector2(   startRatioPosition.X, 0 ) ;

					endRatioPosition = new Vector2( - startRatioPosition.X, startRatioPosition.Y ) ;
				...
				else
				{
					...
					checkRatioPosition = new Vector2( 0,   startRatioPosition.Y ) ;
...
//				bool autoRotation = false ;
				...
				while( true  )	// 画面内の座標割合値で位置を判定する
				{
					...
					if( phase == 0 )
					{
						...
						velocity = checkRatioPosition - startRatioPosition ;
					...
					if( phase == 1 )
					{
						...
						if( factor >= 1 )
						{
							...
							checkRatioPosition = enemy.RatioPosition ;

							velocity = endRatioPosition - checkRatioPosition ;
					...
					if( phase == 2 )
					{
						...
						enemy.RatioPosition = velocity * factor + checkRatioPosition ;
